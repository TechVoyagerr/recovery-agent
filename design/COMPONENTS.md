...
| Gateway connection card | `{ keyId: string; webhookPath: string; webhookHealth: 'ok'\|'failing'; linksCreated24h: number; subscriptions: 'beta'\|'off' }` | `GET /api/settings` (never returns the secret) |
...
| `ArchitectureStrip` | `{}` |, |
| Gateway API grid | `{ apis: { name; body; endpoints }[] }` |, |
...
| `GET/PATCH /api/settings` | Autonomy level, channels, guardrails, payment gateway connection status |
